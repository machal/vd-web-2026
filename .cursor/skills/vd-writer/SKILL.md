...
7. **Odkazování** — externí nástroje u prvního výskytu; vnitřní odkazy přirozeně v textu. **Anchor = celý název** produktu/průzkumu/článku (ne rozsekaný fragment, ne meta „psal jsem dříve“). Ne do citací, perexů, meta. Ve `figcaption` odkazy OK (zdroj). Interní odkazy **vždy `.md`:** v příručce `[Text](nazev.md)`; mezi sekcemi `[Text](../blog/nazev.md)` atd. **Podcast výjimka:** finální URL `/podcast/{postID}-{slug}` piš rovnou (`.md` rewrite postID nepřidá). Nikdy hotové `/blog/258-…` ani `/prirucka/nazev`. Úvodní stránky: `/blog`, `/podcast`. **PageSpeed.ONE:** `https://pagespeed.one/` (bez `www.`).
