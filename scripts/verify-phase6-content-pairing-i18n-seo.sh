...
assert_grep "$CS_WEBP" 'michalek.blog/guide/webp' "CS webp x-default/en target"
...
assert_grep "$EN_GUIDE" 'rel="canonical" href="https://michalek.blog/guide/webp"' "EN webp self canonical"
assert_grep "$EN_GUIDE" 'vzhurudolu.cz/prirucka/webp' "EN webp Česky link target"
assert_grep "$EN_GUIDE" 'aria-label="Also available in Czech."' "EN webp language switch"
...
assert_grep "$CS_DIST/martin/index.html" 'michalek.blog' "CS martin EN footer link"
...
assert_not_grep "$CS_DIST/index.html" 'michalek.blog/guide/' "CS homepage guide pairing link"
